  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error("⚠️ Subscription failed. Please try again later.");
      }
    } catch (err) {
      console.error("Network or fetch error sending to SheetDB:", err);
      // Common cause: CORS blocked by SheetDB. See note below.
      toast.error("⚠️ Could not connect to the signup server. Try again later.");
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-12">
        ...
        <div>
          ...
          <ul className="flex space-x-6 text-xl mb-6">
            ...
            <li>
              <a href="https://www.facebook.com/share/165fra4caq/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className="text-[#F9FAFB] hover:text-[#10B981]">
          ...
          >
            ...
            <button
              type="submit"
              className={`bg-[#10B981] hover:bg-[#0f9d76] text-white px-6 py-2 rounded-lg font-semibold w-full ${sending ? "opacity-70 cursor-not-allowed" : ""}`}
              disabled={sending}
            >
